pub mod dfs {
    ...
    impl Graph {
        ...
        fn dfs(&self, u: usize, visited: &mut HashSet<usize>) {
            visited.insert(u);  // Marking the node as visited and printing the node in the next line
            println!("Visited node: {}", u);

            for &v in &self.edges[u] { // Recursively calling dfs for all unvisited neighbours of the node
